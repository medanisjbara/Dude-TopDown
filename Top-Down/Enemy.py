...
class collider(pygame.sprite.Sprite):
    def __init__(self, spriteEC, x, y, sizeX, sizeY):
        ...
        self.image = spriteEC
        x, y = pygame.Surface.get_width(self.image), pygame.Surface.get_height(
            self.image
        )
        x = x / 2 - sizeX / 2
        y = y / 2 - sizeY / 2
...
class enemy(pygame.sprite.Sprite):
    def __init__(self, spriteE, spriteEC, gamemode):
        ...
        self.X = randint(Cfg.LARGURA / 2, Cfg.LARGURA - self.sizeX - 33)
        self.Y = randint(Cfg.ALTURA / 2, Cfg.ALTURA - self.sizeY - 33)
        self.rect = self.image.get_rect()
        self.rect.topleft = (self.X, self.Y)
        self.speed = Cfg.SPEED_ENEMY
        self.totalLife = Cfg.LIFE_ENEMY
        self.life = Cfg.LIFE_ENEMY
        self.barGreen = 0
        self.barRed = 0
    ...
    # Sets life
    def setLife(self):
        self.totalLife = self.dna[7] * Cfg.LIFE_ENEMY
    ...
    def lifeBar(self, screen):
        self.barGreen = float(self.life) / float(Cfg.LIFE_ENEMY)
        self.barRed = (
            (float(Cfg.LIFE_ENEMY) - float(self.life)) / float(Cfg.LIFE_ENEMY)
        ) * (-1)
        if self.life == Cfg.LIFE_ENEMY:
            pygame.draw.rect(screen, Cfg.VERDE, (self.X + 3, self.Y - 12, 40, 10), 0)
        else:
            if not self.dead:
                pygame.draw.rect(
                    screen,
                    Cfg.VERDE,
                    (self.X + 3, self.Y - 12, self.barGreen * 40, 10),
                    0,
                )
                pygame.draw.rect(
                    screen,
                    Cfg.VERMELHO,
                    (self.X + 43, self.Y - 12, self.barRed * 40, 10),
                    0,
                )
            else:
                pygame.draw.rect(
                    screen, Cfg.VERMELHO, (self.X + 3, self.Y - 12, 40, 10), 0
                )
    ...
    def update(
        ...
        if pygame.sprite.spritecollide(self, healthGroup, True):
            if self.life + Cfg.BONUS_HEALTH <= Cfg.LIFE_ENEMY:
                self.life += Cfg.BONUS_HEALTH
            else:
                self.life = Cfg.LIFE_ENEMY
        # Pts
        ...
        if pygame.sprite.spritecollide(self, shieldGroup, True):
            self.shield_on = True
            shieldImage = pygame.image.load(Cfg.SPRITE_ENEMY_SH).convert_alpha()
            x, y = pygame.Surface.get_width(shieldImage), pygame.Surface.get_height(
                shieldImage
            )
