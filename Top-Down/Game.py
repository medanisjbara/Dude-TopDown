...
def drawpause():
    ...
    text1 = font.render("PAUSE", 1, Cfg.BRANCO)
    text1pos = text1.get_rect()
    text1pos.centerx = screen.get_rect().centerx
    text1pos.centery = screen.get_rect().centery
    screen.blit(text1, text1pos)
    font = pygame.font.Font(None, 36)
    text2 = font.render("Pressione qualquer tecla para continuar", 1, Cfg.BRANCO)
...
def drawmorto():
    font = pygame.font.Font(None, 48)
    text1 = font.render("GAME OVER", 1, Cfg.BRANCO)
    text1pos = text1.get_rect()
    text1pos.centerx = screen.get_rect().centerx
    text1pos.centery = screen.get_rect().centery
    screen.blit(text1, text1pos)
    ...
    txtAgain = font.render("Any key to exit", 1, Cfg.BRANCO)
    txtAgain_rect = txtAgain.get_rect()
    txtAgain_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 2 + 0.1 * Cfg.ALTURA)
...
def drawvitorioso():
    font = pygame.font.Font(None, 48)
    text1 = font.render("VITORIA!", 1, Cfg.BRANCO)
    text1pos = text1.get_rect()
    text1pos.centerx = screen.get_rect().centerx
    text1pos.centery = screen.get_rect().centery
    screen.blit(text1, text1pos)
    ...
    font = pygame.font.Font(None, 30)
    txtAgain = font.render("Play again? (y/n)", 1, Cfg.BRANCO)
    txtAgain_rect = txtAgain.get_rect()
    txtAgain_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 2 + 0.1 * Cfg.ALTURA)
    screen.blit(txtAgain, txtAgain_rect)
    pygame.display.flip()
...
def menuprincipal(gamemode):
    ...
    screen.blit(
        sprite_arrow,
        (Cfg.LARGURA / 2 - 0.33 * Cfg.LARGURA, Cfg.ALTURA / 2 - 0.03 * Cfg.ALTURA),
    )
...
def textoMenuPrincipal():
    ...
    txtTitle = font.render("As aventuras de Dude", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 5)
    screen.blit(txtTitle, txt_rect)
    txtTitle = font.render("no mundo Top Down", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 4)
    screen.blit(txtTitle, txt_rect)

    font = pygame.font.Font("fonts/font.ttf", 18)
    txtTitle = font.render("Modo: Estados Finitos", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 2 - 0.03 * Cfg.ALTURA)
    screen.blit(txtTitle, txt_rect)

    txtTitle = font.render("Modo: Algoritmo Genetico", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 2 + 0.03 * Cfg.ALTURA)
    screen.blit(txtTitle, txt_rect)

    font = pygame.font.Font("fonts/font.ttf", 15)
    txtTitle = font.render("Select mode and press Enter", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 1.2)
    screen.blit(txtTitle, txt_rect)
...
def posmenu():
    screen.fill(Cfg.PRETO)
    font = pygame.font.Font("fonts/font.ttf", 18)
    txtTitle = font.render("Setas - movimenta��o", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 4)
    screen.blit(txtTitle, txt_rect)
    ...
    font = pygame.font.Font("fonts/font.ttf", 18)
    txtTitle = font.render("+" + str(Cfg.BONUS_AMMO) + " muni��es", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, 176)
    screen.blit(txtTitle, txt_rect)

    txtTitle = font.render("+" + str(Cfg.SHIELD_TIME) + "s de imunidade", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, 216)
    screen.blit(txtTitle, txt_rect)

    txtTitle = font.render("+" + str(Cfg.BONUS_HEALTH) + " de vida", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, 256)
    screen.blit(txtTitle, txt_rect)

    txtTitle = font.render("+" + str(Cfg.BONUS_POINTS) + " pontos extra", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, 295)
    screen.blit(txtTitle, txt_rect)

    font = pygame.font.Font("fonts/font.ttf", 15)
    txtTitle = font.render("Press any key to start!", 1, Cfg.BRANCO)
    txt_rect = txtTitle.get_rect()
    txt_rect.midtop = (Cfg.LARGURA / 2, Cfg.ALTURA / 1.2)
    screen.blit(txtTitle, txt_rect)
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if arrow_position == 1:
                        screen.blit(
                            sprite_arrow,
                            (
                                Cfg.LARGURA / 2 - 0.33 * Cfg.LARGURA,
                                Cfg.ALTURA / 2 - 0.03 * Cfg.ALTURA,
                            ),
                        )
...
def startAnimation(previousData, newData, chosenOnes):
    ...
    anim = animation(
        screen, previousData, newData, chosenOnes, enemiesSpritesList, sprite_ec, pygame
    )
...
def status(gamemode, gene1, gene2, gene3, gene4, gene5):
    font = pygame.font.Font("fonts/font.ttf", 18)
    txtStatus = font.render("Vida: " + str(life), 1, Cfg.PRETO, Cfg.BRANCO)
    txtStatusPos = 2, 2
    screen.blit(txtStatus, txtStatusPos)
    font = pygame.font.Font("fonts/font.ttf", 18)
    txtPontos = font.render("Pontos: " + str(pontos), 1, Cfg.PRETO, Cfg.BRANCO)
    txtPontosPos = Cfg.LARGURA2 / 2 - txtPontos.get_width() / 2, 2
    screen.blit(txtPontos, txtPontosPos)
    font = pygame.font.Font("fonts/font.ttf", 18)
    txtAmmo = font.render("Muni��o: " + str(ammo), 1, Cfg.PRETO, Cfg.BRANCO)
    txtAmmoPos = Cfg.LARGURA2 - txtAmmo.get_width(), 2
    screen.blit(txtAmmo, txtAmmoPos)
    font = pygame.font.Font("fonts/font.ttf", 18)
    txtRound = font.render("Round: " + str(Round), 1, Cfg.PRETO, Cfg.BRANCO)
    txtRoundPos = 2, 445
    screen.blit(txtRound, txtRoundPos)
    font = pygame.font.Font(None, 48)
    txtTime = font.render(
        str(minutos).zfill(2) + ":" + str(segundos).zfill(2), 1, Cfg.BRANCO, Cfg.PRETO
    )
    txtTimePos = Cfg.LARGURA2 - txtTime.get_width(), 445
    screen.blit(txtTime, txtTimePos)

    font = pygame.font.Font(None, 18)
    txtGene1 = font.render(str(gene1), 1, Cfg.PRETO, Cfg.BRANCO)
    txtGene1Pos = 6, 520
    screen.blit(txtGene1, txtGene1Pos)
    txtGene2 = font.render(str(gene2), 1, Cfg.PRETO, Cfg.BRANCO)
    txtGene2Pos = 194, 520
    screen.blit(txtGene2, txtGene2Pos)
    txtGene3 = font.render(str(gene3), 1, Cfg.PRETO, Cfg.BRANCO)
    txtGene3Pos = 382, 520
    screen.blit(txtGene3, txtGene3Pos)
    txtGene4 = font.render(str(gene4), 1, Cfg.PRETO, Cfg.BRANCO)
    txtGene4Pos = 570, 520
    screen.blit(txtGene4, txtGene4Pos)
    txtGene5 = font.render(str(gene5), 1, Cfg.PRETO, Cfg.BRANCO)
    txtGene5Pos = 758, 520
...
def gerarRelatorio(listaRound, g, Round, listaCrom):
    ...
    while x < Round:
        ...
        file.write(
            "Tempo de jogo: "
            + str(listaRound[x][2]).zfill(2)
            + ":"
            + str(listaRound[x][3]).zfill(2)
            + "\n"
        )
        file.write("\n")
        if gamemode != 1:
...
def getEnemiesData():
    enemiesData = [
        [DNA1, enemy.getFitness()],
        [DNA2, enemy2.getFitness()],
        [DNA3, enemy3.getFitness()],
        [DNA4, enemy4.getFitness()],
        [DNA5, enemy5.getFitness()],
    ]
    return enemiesData
...
screen = pygame.display.set_mode((Cfg.LARGURA2, Cfg.ALTURA2), 0, 32)
bg = load_image(Cfg.BACKGROUND, "background")
pygame.display.set_caption("TCC - Diego Penha")
pygame.display.set_icon(load_image(Cfg.ICONE, "sprite"))
...
control2 = 1
full_screen = False
pygame.mixer.music.load(Cfg.BACK_MUSIC)
pygame.mixer.music.set_volume(0.6)
pygame.mixer.music.play(-1)
shoot_sound = pygame.mixer.Sound(Cfg.SHOOT_SOUND)
powerup_sound = pygame.mixer.Sound(Cfg.POWERUP_SOUND)
dead_sound = pygame.mixer.Sound(Cfg.DEAD_SOUND)
reload_sound = pygame.mixer.Sound(Cfg.RELOAD_SOUND)
shield_sound = pygame.mixer.Sound(Cfg.SHIELD_SOUND)

# Initializes sprites
sprite_p = load_image(Cfg.SPRITE_PLAYER, "sprite")
sprite_p_sh = load_image(Cfg.SPRITE_PLAYER_SH, "sprite")
sprite_e = load_image(Cfg.SPRITE_ENEMY, "sprite")
sprite_e2 = load_image(Cfg.SPRITE_ENEMY2, "sprite")
sprite_e3 = load_image(Cfg.SPRITE_ENEMY3, "sprite")
sprite_e4 = load_image(Cfg.SPRITE_ENEMY4, "sprite")
sprite_e5 = load_image(Cfg.SPRITE_ENEMY5, "sprite")
sprite_ec = load_image(Cfg.SPRITE_ENEMYC, "collider")
sprite_e_sh = load_image(Cfg.SPRITE_ENEMY_SH, "sprite")
sprite_a = load_image(Cfg.SPRITE_AMMO, "sprite")
sprite_s = load_image(Cfg.SPRITE_SHOT, "background")
sprite_sh = load_image(Cfg.SPRITE_SHIELD, "sprite")
sprite_hp = load_image(Cfg.SPRITE_HEALTH, "sprite")
sprite_pts = load_image(Cfg.SPRITE_POINTS, "sprite")
sprite_arrow = pygame.image.load(Cfg.SPRITE_ARROW).convert_alpha()
enemiesSpritesList = [sprite_e, sprite_e2, sprite_e3, sprite_e4, sprite_e5]

# Powerups initialization
ammoBox = Pups.ammobox(sprite_a)
shield = Pups.Shield(sprite_sh)
health = Pups.Health(sprite_hp)
itemPts = Pups.Points(sprite_pts)
...
if gamemode == 1:
    screen = pygame.display.set_mode((Cfg.LARGURA, Cfg.ALTURA), 0, 32)
elif gamemode == 2:
    screen = pygame.display.set_mode((Cfg.LARGURA2, Cfg.ALTURA2), 0, 32)
...
while playagain:
    ...
    while (life > 0) and (vitoria < 5):
        ...
        segundos = int(
            ((pygame.time.get_ticks() - start_ticks - paused_ticks) / 1000) - 60 * i
        )
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    if not full_screen:
                        screen = pygame.display.set_mode(
                            (Cfg.LARGURA, Cfg.ALTURA), pygame.FULLSCREEN, 32
                        )
                        full_screen = True
                    else:
                        if gamemode == 1:
                            screen = pygame.display.set_mode(
                                (Cfg.LARGURA, Cfg.ALTURA), 0, 32
                            )
                        elif gamemode == 2:
                            screen = pygame.display.set_mode(
                                (Cfg.LARGURA2, Cfg.ALTURA2), 0, 32
                            )
                        full_screen = False
        ...
        life = player.life
        vitoria = (
            enemy.dead2 + enemy2.dead2 + enemy3.dead2 + enemy4.dead2 + enemy5.dead2
        )
        ...
        if pygame.sprite.spritecollide(player, healthGroup, True):
            powerup_sound.play()
            if player.life + health.bonusHP <= Cfg.LIFE_PLAYER:
                player.life += health.bonusHP
            else:
                player.life = Cfg.LIFE_PLAYER
            life = player.life
